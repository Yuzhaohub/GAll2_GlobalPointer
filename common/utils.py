...
class Preprocessor(object):
    ...
    def get_ent2token_spans(self, text, entity_list):
        ...
        inputs = self.tokenizer(text, add_special_tokens = self.add_special_tokens,
                                return_offsets_mapping = True)  # inputs: 表示
        ...
        for ent_span in entity_list:
            ...
            try:
                # 寻找ent的token_span: 查找的原理：通过对文本和label的文本进行分词后，首尾匹配
                ...
                token_start_index = list(
                    filter(lambda x: token2char_span_mapping[x][0] == ent_span[0], token_start_indexs))
