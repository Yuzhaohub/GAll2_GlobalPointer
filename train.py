...
if config["logger"] == "wandb" and config["run_type"] == "train":
    # init wandb: wandb机器学习可视化工具，能够自动记录模型训练过程中的超参数和输出指标，然后可视化和比较结果
        ...
        with open(data_path, encoding = "utf-8") as f:
            for line in f:
                ...
                for k, v in line['label'].items():
                    for spans in v.values():  # 没有包含原始的label文本
...
def data_generator(data_type = "train"):
    ...
    elif data_type == "valid":  # TODO 区别于data_type == 'predicate'
        valid_data_path = os.path.join(config["data_home"], config["exp_name"], config["valid_data"])
        valid_data = load_data(valid_data_path, "valid")
        train_data = []
    ...
    #     tokens = tokenizer(sample["text"])["input_ids"]  # 可以设定死
...
model = GlobalPointer(encoder, ent_type_size, 64)  # 将BertModel作为参数传入，也可以通过继承
